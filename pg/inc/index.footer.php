<hr style="border-top:1px solid #52b69a" class="container-6 m-w-90p">
<div class="container">
    <div class="container-6 m-x-a p-y-7">
        <div class="f-row">
            <div class="f-holder-2 dsp-f f-d-col">
                <span class="m-b-4">Transnational Community Federation e. V.</span>
                <span class="m-b-4">Hieberstraße 11</br>74076 Heilbronn, Germany</span>
                <a class="cl-bright-blue-3 m-b-5" href="mailto:hello@transcf.org">hello@transcf.org</a>
                <span >65209/77790</span>
            </div>
            <div class="f-holder-2 dsp-f jcr jcc-s aic">
                <img src="<?php echo root; ?>content/img/tcf_light.svg" class="w-100p" style="max-width:120px" alt="">
            </div>
        </div>
    </div>
</div>