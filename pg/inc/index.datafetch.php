...
ON tgd.tag_id = tg.ID INNER JOIN blog_tags btg ON tg.ID = btg.tag_id GROUP BY tg.ID ORDER BY tgd.tag ASC, tg.ID ASC");
$stmt->bind_param("s", $l);
$stmt->execute();
$blogTags = $stmt->get_result()->fetch_all(MYSQLI_ASSOC);

$stmt = $con->prepare("SELECT b.*, bd.title, bd.body, bd.descr FROM 
blogs b INNER JOIN (SELECT * FROM blog_detail WHERE lang = ?) bd ON b.ID = bd.blog_id WHERE b.active = 1 GROUP BY b.ID ORDER BY b.ID DESC");
$stmt->bind_param("s", $l);
$stmt->execute();
